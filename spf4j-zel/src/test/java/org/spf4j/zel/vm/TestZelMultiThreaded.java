...
public final class TestZelMultiThreaded {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(TestZelMultiThreaded.class);
   ...
   *
   * // pi launches n goroutines to compute an // approximation of pi. func pi(n int) float64 { ch := make(chan
   * float64) for k := 0; k <= n; k++ { go term(ch, float64(k)) } f := 0.0 for k := 0; k <= n; k++ { f += <-ch } return
   * f }
   *
  ...
  public void testParallelPiImpl() throws CompileException, ExecutionException, InterruptedException {
    ...
    LOG.debug("zel pi par = {} in {} ms", result, (endTime - startTime));
    ...
    LOG.debug("java pi = {} in {} ms", dresult,  (endTime - startTime));
  }
  ...
  public void testParallelPi2Impl() throws CompileException, ExecutionException, InterruptedException, IOException {
    ...
    Number result = (Number) prog.execute(100000);
    long endTime = System.currentTimeMillis();
    LOG.debug("zel pi par optimized = {} in {} ms", result, (endTime - startTime));
    // pi is 3.141592653589793
    Assert.assertEquals(3.141592653589793, result.doubleValue(), 0.0001);
